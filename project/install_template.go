...
const (
	_tplDODockerfile = `FROM hub.jinycoo.com/ubuntu:20.04
RUN mkdir -p /app && cd /app
COPY bin/{{.Name}}-{{.Module}} /app/
EXPOSE 80
ENV DEPLOY_ENV pro
WORKDIR /app/
    ...
    failure {
      emailext(
        ...
        body: """<p>Jenkins build is ${currentBuild.result}: ${env.JOB_NAME} #${env.BUILD_NUMBER}:</p>
            ...
            <p>Check sonarQube code analysis result at <a href="http://192.168.10.188:9000/dashboard?id=${projectName}">project ${projectName}</a></p>""",
...
name: {{.Name}}-{{.Module}}
image:
  repository: 172.16.0.100:5000/{{.Name}}-{{.Module}}
  tag: 1.0.0.d8575b19
