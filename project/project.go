...
func Create() (err error) {
	var gvs = strings.Split(runtime.Version(), ".")
	var gv = strings.Replace(gvs[0], "go", "go ", 1)
	P.GoVersion = strings.Join([]string{gv, gvs[1]}, ".")
	if P.WithGRPC {
	...
	//			_ = os.Chdir(pkg)
	//			ecmd := exec.Command("git", "clone", "https://git.gobies.org/pkg/jinygo.git")
	//			err = ecmd.Run()
	//			if err != nil {
	//				fmt.Println(err)
	//			}
